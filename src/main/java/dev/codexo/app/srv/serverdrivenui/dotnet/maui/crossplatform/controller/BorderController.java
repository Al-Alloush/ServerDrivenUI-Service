...
public class BorderController {
    ...
    @PutMapping("/{borderId}")
    @Operation(
            summary = "Update a border style",
            description = "Updates specific properties of a border style. Only the provided fields will be updated."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Border style updated successfully",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = BorderStyleDto.class)
                    )
            ),
            @ApiResponse(
                    responseCode = "404",
                    description = "Border style not found",
                    content = @Content
            ),
            @ApiResponse(
                    responseCode = "400",
